...
public class DatabaseUpdater {
  ...
  /**
   * This method is to update the latest approve status of given accountId
   * @param accountId the int represents given accound's id
   * @param approve the boolean represents the status
   * @param connection 
   * @return true if the operation success, false otherwise
   */
  protected static boolean updateAccountApprove(int accountId, boolean approve, 
      Connection connection) {
    String sql = "UPDATE ACCOUNT SET APPROVE = ? WHERE ID = ?;";
    try {
      PreparedStatement preparedStatement = connection.prepareStatement(sql);
      
      preparedStatement.setInt(1, approve ? 1 : 0);
      preparedStatement.setInt(2, accountId);
      preparedStatement.executeUpdate();
      return true;
    } catch (Exception e) {
