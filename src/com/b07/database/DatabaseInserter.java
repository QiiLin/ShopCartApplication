...
public class DatabaseInserter {
   ...
   * @throws DatabaseInsertException if something goes wrong.
   */
  protected static int insertAccount(int userId, boolean active, boolean approve, Connection connection) 
      throws DatabaseInsertException {
    String sql = "INSERT INTO ACCOUNT(USERID, ACTIVE, APPROVE) VALUES(?, ?, ?);";
    try {
      PreparedStatement preparedStatement = connection.prepareStatement(sql,
                                              Statement.RETURN_GENERATED_KEYS);
      preparedStatement.setInt(1, userId);
      preparedStatement.setInt(2, active ? 1 : 0);
      preparedStatement.setInt(3, approve ? 1 : 0);
      int id = preparedStatement.executeUpdate();
      if (id > 0) {
        ResultSet uniqueKey = preparedStatement.getGeneratedKeys();
        if (uniqueKey.next()) {
